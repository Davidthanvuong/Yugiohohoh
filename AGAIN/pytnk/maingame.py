...
class Maingame(Component):
    ...
    def update_logic(self):
        if not self.spawned and self.spawnTime.completed:
            ...
            oppo = CardSlot.sides[OPPONENT][2]
            print(player.transf.g_pos, oppo.transf.g_pos)
            Monster.build(player.transf.g_pos, cast('MonsterData', database['King']), self.user1, player)
            Monster.build(oppo.transf.g_pos, cast('MonsterData', database['King']), self.user2, oppo)
